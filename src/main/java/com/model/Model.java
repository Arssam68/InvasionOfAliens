...
public class Model implements Observer {
    ...
    public void removeInactiveObjects() {
        ...
        while (aliensIterator.hasNext()) {
            if (!aliensIterator.next().isActive()) {
                aliensIterator.remove();
                /*if (Alien.getCurrentNumber() <= Game.MAX_NUMBER_OF_ALIANS) {
                    Alien alien = new Alien((int) (Math.random() * (Game.WIDTH - Alien.WIDTH - 1) + Alien.WIDTH / 2), (int) (Math.random() * 2 + 1), 1);
                    alien.addObserver(this);
                    aliens.add(alien);
                }*/
            }
